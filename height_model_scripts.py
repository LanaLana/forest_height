...
class Generator:
    ...
    def get_img_mask_array(self, imgpath):
        ...
        if self.arctic_dem:
            ...
            img[:,:,len(self.channels_list)] = self.channel_resize(imgpath, channel_name, window_x0, window_y0, 
                                        window_x1, window_y1, IMG_ROW, IMG_COL) 
            img[:,:,len(self.channels_list)] = img[:,:,len(self.channels_list)].clip(self.arctic_stat['min'], self.arctic_stat['max']) + 169.93*np.where(img[:,:,len(self.channels_list)]==0,1, 0)
    ...
    def split_img(self, img, IMG_ROW, IMG_COL, overlap):
        height_ind = 0
        height = 0
        imgarr = []
    ...
    def reconstruct_pred(self, pred, size_x, size_y, IMG_ROW, IMG_COL, overlap, height_ind, width_ind):
    ...
    def pred_img(self, model, img_path, num_classes=1, inference = False, overlap = 128, crop_size = 512): 
        ...
        imgarr, height_ind, width_ind = self.split_img(img, IMG_ROW, IMG_COL, overlap)
        
        pred = np.asarray([model.predict(np.asarray([img_patch]))[0] for img_patch in imgarr])
        recon = self.reconstruct_pred(pred, size_x, size_y, IMG_ROW, IMG_COL, overlap, height_ind, width_ind)
